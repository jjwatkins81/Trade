"""Simple market-sentiment dashboard: indicators, news sentiment, gamma exposure,
...
from src.backtest import STRATEGIES, fetch_history, run_backtest
...
tab_overview, tab_backtest = st.tabs(["📊 Market Overview", "🧪 Strategy Backtester"])
...
with tab_overview:
    ...
    # fragment so changing them triggers a full rerun (needed to pick up a
    # new auto-refresh interval or ticker list).
    # -----------------------------------------------------------------------
...
with tab_backtest:
    ...
    st.caption(
        "Test a rule-based strategy against historical prices for any ticker, and see "
        "how its buy/sell signals would have performed vs. simply buying and holding."
    )
    ...
    with st.form("backtest_form"):
        ...
        if strategy_spec["params"]:
            param_cols = st.columns(len(strategy_spec["params"]))
            for col, (key, (label, lo, hi, default)) in zip(param_cols, strategy_spec["params"].items()):
                with col:
                    step = 1 if isinstance(default, int) else 0.1
                    params[key] = col.slider(label, lo, hi, default, step=step)

        c1, c2 = st.columns(2)
        with c1:
            initial_capital = st.number_input(
                "Starting capital ($)", min_value=100.0, value=BACKTEST_INITIAL_CAPITAL, step=1000.0
            )
        with c2:
            commission_pct = st.number_input(
                "Commission per trade (%)", min_value=0.0, value=BACKTEST_COMMISSION_PCT, step=0.01
            )
    ...
    if submitted:
        if start_date >= end_date:
            ...
            with st.spinner(f"Fetching {ticker} history and running the backtest..."):
                try:
                    history = fetch_history(ticker, start_date, end_date)
                except Exception as exc:
                    history = pd.DataFrame()
                    st.error(f"Couldn't fetch history for {ticker}: {exc}")

            if history.empty or "Close" not in history:
                st.error(f"No price data found for '{ticker}' in that date range.")
            else:
                position = strategy_spec["fn"](history, **params)
                result = run_backtest(
                    history, position, initial_capital=initial_capital, commission_pct=commission_pct
                )

                st.subheader("Performance")
                m1, m2, m3, m4, m5 = st.columns(5)
                m1.metric("Total return", f"{result.total_return_pct:+.1f}%", delta=f"{result.total_return_pct - result.benchmark_return_pct:+.1f}% vs. buy & hold")
                ...
                m5.metric("Win rate", f"{result.win_rate_pct:.0f}% ({result.num_trades} trades)")
                ...
                entries = result.position.diff() > 0
                exits = result.position.diff() < 0

                price_fig = go.Figure()
                price_fig.add_trace(
                    go.Scatter(x=history.index, y=history["Close"], name="Close", line=dict(color="#4c78a8"))
                )
                price_fig.add_trace(
                    go.Scatter(
                        x=history.index[entries],
                        y=history["Close"][entries],
                        mode="markers",
                        name="Buy",
                ...
                price_fig.add_trace(
                    go.Scatter(
                        x=history.index[exits],
                        y=history["Close"][exits],
                        mode="markers",
                        name="Sell",
                        marker=dict(symbol="triangle-down", size=11, color="#d62728"),
                    )
                )
                price_fig.update_layout(height=420, yaxis_title="Price ($)", margin=dict(l=20, r=20, t=30, b=10))
                ...
                equity_fig.add_trace(
                    go.Scatter(x=result.equity_curve.index, y=result.equity_curve, name=strategy_name, line=dict(color="#2ca02c"))
                )
                equity_fig.add_trace(
                    go.Scatter(x=result.benchmark_curve.index, y=result.benchmark_curve, name="Buy & Hold", line=dict(color="#999999", dash="dash"))
                )
                equity_fig.update_layout(height=350, yaxis_title="Portfolio value ($)", margin=dict(l=20, r=20, t=30, b=10))
                ...
                if result.trades:
                    ...
                    st.info("This strategy never generated a buy signal in the selected date range.")

                st.caption(
                    "Backtest only — past performance on historical data does not predict future "
                    "results. Signals execute at the next bar's close after they trigger (no "
                    "look-ahead), and commission is applied on every entry/exit."
                )
    else:
        st.info("Pick a ticker and strategy above, then run the backtest.")
